...
class ParamWithNamePredicate:
    """Wraps a matching function (param + name) to make it hashable for ParamKey.

    Example:
        >>> shape_1_not_qkln = ParamWithNamePredicate(
        ...     name="s1_not_qkln",
        ...     fn=lambda param, name: (len(param.shape) == 1 or name.endswith(".bias"))
        ...         and not ("q_layernorm." in name or "k_layernorm." in name),
        ... )
        >>> shape_1_not_qkln(torch.empty(10), "module.bias")
        True
        >>> shape_1_not_qkln(torch.empty(10), "module.q_layernorm.bias")
        False
...
class ParamKey:
    ...
    """Parameter name(s), matched with unix filesystem path syntax (fnmatch)."""
    ...
    """Predicate(s) to match parameters by. Any one match is sufficient."""

    with_name_predicate: Union[ParamWithNamePredicate, Tuple[ParamWithNamePredicate]] = field(
        default_factory=tuple
    )
    """Predicate(s) to match parameters with their name. Any one match is sufficient."""

    def matches(self, param: torch.nn.Parameter, param_name: str) -> bool:
        """Returns true if the parameter (with name) matches this ParamKey.
        ...
        Returns:
            bool: True if parameter matches.
        """
        # Check if name matches (glob/fnmatch patterns supported, e.g. "*.bias").
        target_names = [self.name] if isinstance(self.name, str) else list(self.name)
        for target_name in target_names:
            if fnmatch.fnmatch(param_name, target_name):
                return True

        # Check if attribute matches.
        target_attrs = [self.attr] if isinstance(self.attr, str) else list(self.attr)
        ...
        predicates = (
            [self.predicate] if isinstance(self.predicate, ParamPredicate) else list(self.predicate)
        )
        ...
        with_name_predicates = (
            [self.with_name_predicate]
            if isinstance(self.with_name_predicate, ParamWithNamePredicate)
            else list(self.with_name_predicate)
        )
        for predicate in with_name_predicates:
...
class OptimizerConfig:
    ...
    """If true, apply weight decay to q_layernorm and k_layernorm parameters as a special case.
    By default (False), all 1-D parameters and biases skip weight decay. When True, 1-D params
    and biases that belong to q_layernorm / k_layernorm still receive weight decay.
    """
