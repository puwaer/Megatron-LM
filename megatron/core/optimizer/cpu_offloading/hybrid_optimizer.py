...
class HybridDeviceOptimizer(torch.optim.Optimizer):
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                if is_float8tensor(orig_param):
                    ...
                    master_dtype = self._master_dtype if self._master_dtype is not None else torch.float32
                    master = orig_param.float().to(master_dtype).detach().clone()
    ...
    def _update_fp32_params_by_new_state(self):
        if self._master_dtype is None and not self.param_update_in_fp32:
            return
    ...
    def _register_load_state_dict_hooks(self):
        def pre_load_state_dict_hook(self, state_dict):
            ...
            """
            if not (self.param_update_in_fp32 or self._master_dtype is not None):
                return state_dict
        ...
        def post_load_state_dict_hook(self):
            ...
            if self.param_update_in_fp32 or self._master_dtype is not None:
                new_state = {}
                for param, v in self.state.items():
